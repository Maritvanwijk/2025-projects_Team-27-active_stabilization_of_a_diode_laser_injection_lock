...
with open("current5002.csv", "r") as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
        try:
...
# Genormaliseerde en omgekeerde intensiteit (relatief)
y_fit_norm = (y_fit - np.min(y_fit)) / (np.max(y_fit) - np.min(y_fit))  # tussen 0 en 1
...
plt.title("Laserspectrum bij 50.02 mA")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
